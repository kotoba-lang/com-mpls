(ns mpls.main
  "Kotodama WASM entrypoint for the Mpls clean-room actor (L4) — Clojure port.
  ...
  facts (`mpls.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "LabelStackEntry"     :plural "labelstackentries"     :id-prefix "mpls_lab"
    :fields [:label :tc :s :ttl]                  :required [:label :tc]
    :coerce {:label :int :tc :int :s :bool :ttl :int}  :refs {}}
   {:entity "MplsHeader"           :plural "mplsheaders"           :id-prefix "mpls_mpl"
    :fields [:stackDepth :topLabel :trafficClass :bottomOfStack :timeToLive] :required [:stackDepth :topLabel]
    :coerce {:stackDepth :int :topLabel :int :trafficClass :int :bottomOfStack :bool :timeToLive :int} :refs {}}
   {:entity "ReservedLabel"        :plural "reservedlabels"        :id-prefix "mpls_res"
    :fields [:value :name :purpose]               :required [:value :name]
    :coerce {:value :int}                         :refs {}}
   {:entity "NhlfeOperation"       :plural "nhlfeoperations"       :id-prefix "mpls_nhl"
    :fields [:operationType :newLabel :nextHopDestination] :required [:operationType :newLabel]
    :coerce {:newLabel :int}                      :refs {}}
   {:entity "MplsForwardingState"  :plural "mplsforwardingstates"  :id-prefix "mpls_mpl"
    :fields [:incomingLabel :fecRef :hopCount]   :required [:incomingLabel :hopCount]
    :coerce {:incomingLabel :int :hopCount :int}         :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"mpls.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mpls-compat" :tier tier :entities entities} 200])
